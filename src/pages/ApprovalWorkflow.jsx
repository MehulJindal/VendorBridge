    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Procurement authorization & sign-off tracker
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          <Card className="p-5">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <p className="text-slate-400 text-sm">
                  {apr.id} · {apr.rfqId} · Requested by {apr.requestedBy}
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ["Request Date", apr.requestedAt],
                ["Selected Vendor", apr.selectedVendor],
                ["Order Value", fmt(apr.amount)],
                ["Approval Level", "L2 – Manager"],
              ].map(([l, v]) => (
              ...
              <div className="space-y-0">
                {apr.steps.map((step, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`relative z-10 w-12 h-12 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${step.status === "completed" ? "bg-emerald-500/10 border-emerald-500 text-emerald-400" : step.status === "pending" ? "bg-amber-500/10 border-amber-500 text-amber-400 animate-pulse" : "bg-slate-900 border-slate-700 text-slate-600"}`}
                      >
                        ...
                        {isCurrentPending && !approved && (
                          <div className="mt-3 space-y-3">
                            <textarea
                              ...
                              className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500 resize-none"
                            />
                            <div className="flex gap-2">
                              <button
                                onClick={() => handleAction("approve")}
                                className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg text-xs font-medium transition-colors"
                              >
                                <Check size={12} />
                                Approve
                              </button>
                        ...
                        {isCurrentPending && approved && (
                          <div className="mt-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3 flex items-center gap-2">
                            ...
                            <span className="text-emerald-400 text-xs font-medium">
                              Approved — Forwarded to Admin for final sign-off
                            </span>
                          </div>
                        )}
                      </div>
        ...
        <div className="space-y-5">
          <Card className="p-5">
            ...
            <div className="space-y-2">
              {[
                ...
                <div
                  key={item.id}
                  className={`p-3 rounded-lg border cursor-pointer hover:border-slate-600 transition-colors ${item.id === "APR-0087" ? "bg-indigo-500/5 border-indigo-500/30" : "bg-slate-900 border-slate-800"}`}
                >
          ...
          <Card className="p-5">
            ...
            <div className="space-y-3">
              {[
                ["Approved (30d)", "23", "text-emerald-400"],
                ["Rejected (30d)", "4", "text-red-400"],
                ["Avg. Approval Time", "1.8 Days", "text-indigo-400"],
                ["Pending Right Now", "4", "text-amber-400"],
              ].map(([label, val, cls]) => (
