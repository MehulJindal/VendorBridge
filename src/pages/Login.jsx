      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 bg-gradient-to-br from-slate-950 via-indigo-950/40 to-slate-950 p-16 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-slate-500 text-sm">
            Procurement & Vendor ERP Platform
          </p>
        </div>
          ...
          {[
            {
              ...
              desc: "From RFQ creation to PO issuance — one unified platform.",
            },
            {
              icon: Zap,
              title: "Real-Time Bid Comparison",
              desc: "Side-by-side vendor quotation analytics with cost intelligence.",
            },
            {
              icon: Target,
              title: "Multi-Level Approvals",
              desc: "Configurable approval chains with complete audit trails.",
            },
        ...
        <div className="relative z-10">
          <p className="text-slate-600 text-xs">
            © 2024 VendorBridge Corp. Enterprise Edition v4.2.1
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors pr-10"
                    />
                    <button
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {roles.map(({ id, label, icon: Icon }) => (
                      <button
                        key={id}
                        onClick={() => setRole(id)}
                        className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border text-xs font-medium transition-all ${role === id ? "bg-indigo-600/20 border-indigo-500 text-indigo-300" : "bg-slate-800/50 border-slate-700 text-slate-400 hover:border-slate-600"}`}
                      >
                        <Icon size={14} />
                        {label.split(" ")[0]}{" "}
                        {label.split(" ").slice(1, 2).join(" ")}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          {mode === "signup" && (
            <>
              ...
              <div className="space-y-4">
                {[
                  ["Full Name", "text", "Suresh Kumar"],
                  ["Work Email", "email", "suresh@company.in"],
                  ["Company / Organization", "text", "ABC Enterprises"],
              ...
              <div className="space-y-4">
                ...
                <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-lg p-4">
                  <p className="text-indigo-300 text-xs">
                    A one-time reset link will be sent to the registered email.
